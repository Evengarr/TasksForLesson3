...
public class Database {
    ...
    public static boolean connect() {
        try {
            ...
            connection = DriverManager.getConnection("jdbc:sqlite:Lesson2/Server/chat.db");
    ...
    public static void prepareAllStatement() throws SQLException {
        createUserStatement = connection
                .prepareStatement("INSERT INTO user (" +
                        "login, password, nickname) VALUES (?, ?, ?);create table user(	login int,	password int,	nickname int);");
        getUserNicknameStatement = connection
                .prepareStatement("SELECT nickname FROM Users WHERE login = ? AND password = ?;");
        changeUserNicknameStatement = connection
    ...
    public static String getUserNickname(String login, String password) {
        ...
        try {
            getUserNicknameStatement
                    .setString(1, login);
            getUserNicknameStatement
                    .setString(2, password);
            ResultSet rs = getUserNicknameStatement
                    .executeQuery();
            if (rs.next()) {
    ...
    public static boolean changeUserNickname(String currentNickname, String newNickname) {
        try {
            changeUserNicknameStatement
                    .setString(1, newNickname);
            changeUserNicknameStatement
                    .setString(2, currentNickname);
            changeUserNicknameStatement
                    .executeUpdate();
            return true;
        } catch (SQLException e) {
